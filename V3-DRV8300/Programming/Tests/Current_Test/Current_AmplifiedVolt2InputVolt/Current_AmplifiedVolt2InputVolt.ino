...
void loop()
{
    Serial3.print(Amplified_Volt2Input_Volt(analogRead(SO1) / 1023. * 3.3), 6); // be careful of the resolution. In the case, the analogRead() Function resolution is 1023
    Serial3.print("   ");
    Serial3.println(Amplified_Volt2Input_Volt(analogRead(SO2) / 1023. * 3.3), 6);
}

double Amplified_Volt2Input_Volt(double Amplified_Volt)
{
    const double K = 2.;
    const double G = 10.;
    ...
    return ((VREF - (K * Amplified_Volt)) / (G * K));
}