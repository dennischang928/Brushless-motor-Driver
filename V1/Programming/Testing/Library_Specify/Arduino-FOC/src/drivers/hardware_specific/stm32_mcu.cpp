...
#define _PWM_RESOLUTION 8 // 12bit
#define _PWM_RANGE 5.0 // 2^12 -1 = 4095
...
#define _PWM_RESOLUTION 8 // 12bit
#define _PWM_RANGE 255 // 2^12 -1 = 4095
#define _PWM_FREQUENCY 25000 // 25khz
#define _PWM_FREQUENCY_MAX 50000 // 50khz
...
void _writeDutyCycle6PWM(float dc_a,  float dc_b, float dc_c, float dead_zone, int pinA_h, int pinA_l, int pinB_h, int pinB_l, int pinC_h, int pinC_l){
  ...
  _setPwm(pinA_h, constrain(dc_a - dead_zone/2, 0, 1)*_PWM_RANGE, _PWM_RESOLUTION);
  // _setPwm(pinB_l, _constrain(dc_b + dead_zone/2, 0, 1)*_PWM_RANGE, _PWM_RESOLUTION);
  _setPwm(pinB_h, constrain(dc_b - dead_zone/2, 0, 1)*_PWM_RANGE, _PWM_RESOLUTION);
  // _setPwm(pinC_l, _constrain(dc_c + dead_zone/2, 0, 1)*_PWM_RANGE, _PWM_RESOLUTION);
  _setPwm(pinC_h, constrain(dc_c - dead_zone/2, 0, 1)*_PWM_RANGE, _PWM_RESOLUTION);
}

void _ResetPin(int dc_a, int dc_b, int dc_c){
  digitalWrite(dc_a, LOW);
  digitalWrite(dc_b, LOW);
  digitalWrite(dc_c, LOW);
}